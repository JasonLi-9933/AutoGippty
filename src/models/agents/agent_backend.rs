...
use crate::helpers::command_line::*;
...
impl AgentBackendDeveloper {
    ...
    async fn call_fix_code_bug(&mut self, factsheet: &mut FactSheet) {
        let msg_context: String = format!(
            "BROKEN_CODE: {:?} \n ERROR_BUGS: {:?} \n
    ...
    // print out rest API for frontend (potentially)
            ...
            match self.attributes.state {
                ...
                AgentState::Working => {
                    if self.bug_count == 0 {
                        ...
                        self.call_fix_code_bug(factsheet).await;
                    }
                ...
                AgentState::UnitTesting => {
                    ...
                    let is_safe_to_continue = confirm_safe_code();
                    if !is_safe_to_continue {
                        panic!("I don't trust AI now, I want to write code myself!");
                    }
                    ...
                    if build_backend_server.status.success() {
                        ...
                        PrintCommand::UnitTest.print_agent_message(
                            self.attributes.position.as_str(),
                            "Backend Code Build Success ",
                        );
                    } else {
                        let error_vec: Vec<u8> = build_backend_server.stderr;
                        ...
                        if self.bug_count > 2 {
                            PrintCommand::Issue.print_agent_message(
                                self.attributes.position.as_str(),
                                "Backend Code Unit Testing: Backend Code Has Too Many Bugs! ",
                            );
                            panic!("Error: Too Many Bugs in Backend Code")
                        }
                    ...
                    let check_endpoints: Vec<RouteObject> = api_endpoints
                        .iter()
                        .filter(|&r| r.method == "get" && r.is_route_dynamic == "false")
                        .cloned()
                        .collect();
                    ...
                    // sleep 5 secs to wait for the server to boot up
