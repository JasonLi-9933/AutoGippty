...
impl AgentSolutionArchitect {
    ...
    async fn call_project_scope(&mut self, factsheet: &mut FactSheet) -> ProjectScope {
        ...
        let ai_response: ProjectScope = ai_task_request_decoded::<ProjectScope>(
            ...
            get_function_string!(print_object_scope),
            print_project_scope,
        )
        .await;
        ...
        while self.attributes.state != AgentState::Finished {
            match self.attributes.state {
                AgentState::Discovery => {
                    ...
                    // Confirm if external urls exist
                ...
                AgentState::UnitTesting => {
                    let mut exclude_urls: Vec<String> = vec![]; // urls don't work
                    let client: Client = Client::builder()
                        .timeout(Duration::from_secs(5))
                        .build()
                        .unwrap();

                    // define urls to be checked
                ...
                // Default to set state to Finished
                _ => {
                    self.attributes.state = AgentState::Finished;
                }
            }
...
mod tests {
    ...
    async fn tests_solution_architect() {
        ...
        assert!(factsheet.external_urls != None);
